...
export async function POST(
  ...
  const [backendResponse, fetchError] = await catchError(
    fetch(`${BACKEND_URL}/${commentId}/likes`, { method: 'POST', headers: authHeader }),
  );
  if (fetchError) {
  ...
  const result = await parseApiEnvelope<LikeResponse>(backendResponse, '無法對留言按讚');
...
export async function DELETE(
  ...
  const [backendResponse, fetchError] = await catchError(
    fetch(`${BACKEND_URL}/${commentId}/likes`, { method: 'DELETE', headers: authHeader }),
  );
  if (fetchError) {
    return NextResponse.json(
      {
  ...
  const result = await parseApiEnvelope<LikeResponse>(backendResponse, '無法取消留言按讚');
  return NextResponse.json(result, { status: backendResponse.status });
}
