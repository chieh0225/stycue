...
export async function PUT(
  ...
  const result = await parseApiEnvelope<CommentResponse>(backendResponse, '無法更新留言');
...
export async function DELETE(
  ...
  const result = await parseApiEnvelope<unknown>(backendResponse, '無法刪除留言');
  return NextResponse.json(result, { status: backendResponse.status });
}
