...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [backendResponse, fetchError] = await catchError(
    fetch(`${BACKEND_URL}/${id}/favorites`, { method: 'POST', headers: authHeader }),
  );
  if (fetchError) {
  ...
  const result = await parseApiEnvelope<FavoriteResponse>(backendResponse, '無法收藏委託文');
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [backendResponse, fetchError] = await catchError(
    fetch(`${BACKEND_URL}/${id}/favorites`, { method: 'DELETE', headers: authHeader }),
  );
  if (fetchError) {
    return NextResponse.json(
      {
  ...
  const result = await parseApiEnvelope<FavoriteResponse>(backendResponse, '無法取消收藏委託文');
  return NextResponse.json(result, { status: backendResponse.status });
}
