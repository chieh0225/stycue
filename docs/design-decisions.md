...
4. 全站 150+ 處字級呼叫點（含舊 7 階 token 與 arbitrary 值）依**實際語意角色**而非機械式就近像素值重新對應，例如：留言/回覆內文從 14–14.5px 提升到 `body-lg`(16px)（因為內文本該是主要可讀內容，貼近就近值反而會誤判成次要文字）；Modal/Sheet 標題原本分散在 3 種不同尺寸，收斂成單一 `headline-sm`(20px) 烘進 `DialogTitle`/`SheetTitle` 元件本身。（原本此處還列了「簽到點數等『大型強調數字』提升到 `headline-md`(24px)」一項，經比對官方設計提案後確認是誤判——簽到彈窗的積分數字應為 `body-lg`(16px)，已更正，見簽到 modal 收尾調整。）
